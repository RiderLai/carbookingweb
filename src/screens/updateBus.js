...
import { http, driverListAPI, busUpdateAPI, busIdAPI } from '../api/api';
...
export default function UpdateBus(props) {
	...
	const initData = async () => {
		...
		if (busResp.data.meta.success) {
			setName(busResp.data.data.name);
			setLicence(busResp.data.data.licence);
			setPassengerMax(busResp.data.data.passengerMax);
			setDriveValue(busResp.data.data.driverId);
			setDepartureTime(new Date(busResp.data.data.departureTime));
			setRoute(busResp.data.data.route);
			setType(busResp.data.data.type);
		}
	};
